...
var rl, vs, r, vd1, vd2, vd3, id1, id2, vt, isc, irev;
var n = 1.2, rd = 0.3, vr = 30, k = 273, dn = 11600, rtmax = 100, rtmin = 26, iscmax = 50, iscmin = 5, isc1 = 0.000000000001;

function rbias() {
  vs = document.getElementById("dc-value").value;
  r = document.getElementById("rl-value").value;
  ...
  vd1 = (parseFloat(vs) / ((parseInt(rl))+parseFloat(rd)));
  vd2 = Math.log(vd1 / isc);
  vd3 = vs * n * vt * vd2;
  id1 = parseFloat(isc1) * Math.pow(10, 11);
  id = -(id1); //
  vd = -(vd3) //Reverse Voltage across the diode in VOLTS
  document.getElementById("am-value").value = id.toPrecision(3);
  document.getElementById("vm-value").value = vd.toPrecision(3);

  if (vs > parseFloat(vr)) {
    id2 = (parseFloat(vs) / ((parseFloat(rl))+parseFloat(rd)));
    id = -(id2);
    document.getElementById("am-value").value = id.toPrecision(3);
  }
}
...
function chngtemp() {
  ...
  document.getElementById("dc-slider").value = "0";
